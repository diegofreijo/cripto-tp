...
class ManoTruco:
  ...
  def __init__(self, cartasIniciales, soyMano):
    ...
    self._cartasQueTengo = cartasIniciales[:] # copiar la lista!
    self._cartasJugoOtro = []
  ...
  def jugadasPosibles(self):
    if self.terminado():
      return [] # el juego termino, ya no hay jugadas posibles
  ...
  def jugar( self, laJugada ):
    ...
    if self.terminado() or not self._esMiTurno:
      return False
    ...
    if isinstance( laJugada, Canto.Canto ):
      if laJugada.tipoCanto == Canto.AL_MAZO:
        ...
        self._ganeYo = False
        return True
      return False # si no es AL_MAZO, no es una jugada valida

    # asumo que  laJugada es una carta
    if laJugada not in self._cartasQueTengo:
      return False # se intento jugar con una carta que no se tiene o que ya se jugo.
    self._juegoMio = laJugada
    self._cartasQueTengo.remove( self._juegoMio )
  ...
  def recibirJugada( self, laJugada ):
    if self.terminado() or self._esMiTurno:
      return False
    ...
    if laJugada in self._cartasJugoOtro:
      return False # esa carta ya la jugo mi contrincante, no puede volver a jugarse
    if laJugada in self._cartasQueTengo:
      return False # es una de mis cartas por jugar
    self._cartasJugoOtro.append(laJugada)
  ...
  def _mata(self, carta1, carta2):
    return self._valorRelativo(carta1) > self._valorRelativo(carta2)
  ...
  def _terminaSubMano( self ):
    ...
    if self._mata(self._juegoMio, self._juegoOtro ):
      #print str(self._juegoMio) + ' > ' + str(self._juegoOtro)
    ...
    elif self._mata(self._juegoOtro, self._juegoMio):
      #print str(self._juegoMio) + ' < ' + str(self._juegoOtro)
      if self._subManoActual == 1:
      ...
      #print str(self._juegoMio) + ' parda con ' + str(self._juegoOtro)
